...
function getHtmlToCanvas(window, document, exports, global, define, undefined) {
    ...
    Ar.html2canvas = function (nodeList, options) {
        var index = html2canvasCloneIndex++;
        options = options || {};
        if (options.logging) {
            Ar.html2canvas.logging = true;
            Ar.html2canvas.start = Date.now();
        }
    ...
    Ar.html2canvas.punycode = this.punycode;
    Ar.html2canvas.proxy = {};
    ...
    function log() {
        if (Ar.html2canvas.logging && window.console && window.console.log) {
            Function.prototype.bind.call(window.console.log, (window.console)).apply(window.console, [(Date.now() - Ar.html2canvas.start) + "ms", "html2canvas:"].concat([].slice.call(arguments, 0)));
        }
    }
    ...
    NodeParser.prototype.paintText = function (container) {
        ...
        var characters = Ar.html2canvas.punycode.ucs2.decode(container.node.data);
        var textList = (!this.options.letterRendering || noLetterSpacing(container)) && !hasUnicode(container.node.data) ? getWords(characters) : characters.map(function (character) {
            return Ar.html2canvas.punycode.ucs2.encode([character]);
        });
    ...
    function getWords(characters) {
        ...
        while (characters.length) {
            if (isWordBoundary(characters[i]) === onWordBoundary) {
                ...
                if (word.length) {
                    words.push(Ar.html2canvas.punycode.ucs2.encode(word));
                }
    ...
    function jsonp(document, url, callback) {
        return new Promise(function (resolve, reject) {
            ...
            var cleanup = function () {
                delete Ar.html2canvas.proxy[callback];
                document.body.removeChild(s);
            };
            Ar.html2canvas.proxy[callback] = function (response) {
                cleanup();
                resolve(response);
            };
            s.src = url;
